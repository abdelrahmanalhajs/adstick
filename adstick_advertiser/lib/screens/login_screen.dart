...
class _LoginScreenState extends State<LoginScreen> {
  final _email = TextEditingController(text: 'brand@mybrand.sa');
  ...
  Widget build(BuildContext context) => Scaffold(
    ...
    body: SafeArea(child: SingleChildScrollView(padding: const EdgeInsets.all(28), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      ...
      RichText(text: const TextSpan(children: [
        TextSpan(text: 'Ad', style: TextStyle(fontSize: 30, fontWeight: FontWeight.w900, color: AppTheme.brand)),
        TextSpan(text: 'Stick', style: TextStyle(fontSize: 30, fontWeight: FontWeight.w900, color: Colors.white)),
      ])),
      const SizedBox(height: 4),
      const Text('Advertiser Portal', style: TextStyle(color: AppTheme.textMuted, fontSize: 15)),
      const SizedBox(height: 48),
      const Text('Welcome back 👋', style: TextStyle(fontSize: 26, fontWeight: FontWeight.w900, color: Colors.white)),
      const SizedBox(height: 6),
      const Text('Sign in to manage your campaigns', style: TextStyle(color: AppTheme.textMuted, fontSize: 14)),
      ...
      const SizedBox(height: 16),
      Center(child: TextButton(onPressed: () {}, child: const Text('New advertiser? Create account →', style: TextStyle(color: AppTheme.brand)))),
    ]))),
  );

  Widget _field(String label, TextEditingController ctrl, IconData icon, {bool obscure = false}) => Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
